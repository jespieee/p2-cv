...
void Image_init(Image* img, std::istream& is) {
	...
	int maxIntensity;
	int r, g, b;
	is >> fileType >> width >> height >> maxIntensity;
	img->width = width;
	img->height = height;
	Matrix_init(&img->red_channel, width, height);
	Matrix_init(&img->green_channel, width, height);
...
Pixel Image_get_pixel(const Image* img, int row, int column) {
	Pixel returnPixel;
	returnPixel.r = *Matrix_at(&img->red_channel, row, column);
	returnPixel.g = *Matrix_at(&img->green_channel, row, column);
	returnPixel.b = *Matrix_at(&img->blue_channel, row, column);
	return returnPixel; // TODO Replace with your implementation!
}
