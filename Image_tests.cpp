...
TEST(test_image_dimensions) {
	...
	int height1 = 3;
	Image_init(img, 3, 3);
	int width2 = Image_width(img);
	int height2 = Image_height(img);
...
TEST(test_image_fill) {
	Image* img = new Image;
	Image_init(img, 2, 2);
	Pixel setter = { 255, 0, 255 };
	Image_fill(img, setter);
	
	ASSERT_TRUE(Pixel_equal(Image_get_pixel(img, 1, 1), setter));
